...
if ($_POST) {
    ...
    if (!isset($error)) {
        try {
            $query = "UPDATE events SET title = :title, description = :description, organizer = :organizer, 
                     location = :location, event_date = :event_date, start_time = :start_time, 
                     end_time = :end_time, max_participants = :max_participants, status = :status, 
...
<head>
    ...
    <title>Edit Event - VolunteerHub Admin</title>
    ...
    <div class="main-content">
        ...
        <header class="top-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 fw-semibold text-dark mb-1">Edit Event</h1>
                    <p class="text-muted mb-0">Perbarui informasi event volunteer</p>
                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="max_participants" name="max_participants" min="1" value="<?php echo $event['max_participants']; ?>" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="status" name="status" required>
                                            <option value="active" <?php echo ($event['status'] == 'active') ? 'selected' : ''; ?>>Aktif</option>
                                            <option value="inactive" <?php echo ($event['status'] == 'inactive') ? 'selected' : ''; ?>>Tidak Aktif</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <?php if ($event['image_path'] && file_exists('../' . $event['image_path'])): ?>
                                    <div class="mb-3">
                                        ...
                                        <img src="../<?php echo htmlspecialchars($event['image_path']); ?>" alt="Current Image" class="current-image">
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="upload-area" id="uploadArea">
                                    ...
                                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="document.getElementById('event_image').click()">
                                        <?php echo ($event['image_path']) ? 'Ganti Gambar' : 'Pilih Gambar'; ?>
                                    </button>
                                </div>
